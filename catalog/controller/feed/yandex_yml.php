<?php
/**
* Yandex.YML data feed for OpenCart (ocStore) 2.x
*
 ...
 */
//Версия модуля
define ('YANDEX_YML_VERSION', '1.8.4');
...
//---- Config section ----
	...
	public function saveToFile($dirname) {
		$filename = $dirname[0] . '/' . $this->CONFIG_PREFIX . $this->config->get($this->CONFIG_PREFIX.'token') . '.xml';
		$fp = fopen($filename, 'w');
		if ($this->config->get($this->CONFIG_PREFIX.'status')) {
			$this->putYml($fp);
		}
...
//				$data['cbid'] = 15;
			...
			if ($product['shipping']) {
				...
				if (is_array($this->delivery_option) && isset($this->delivery_option['cost'])) {
					...
					if ($data['available']) {
						if (isset($yandex_yml_categ_delivery_days[$product['category_id']]) && $yandex_yml_categ_delivery_days[$product['category_id']]!='') {
			...
			elseif (isset($yandex_yml_categ_cpa) && count($yandex_yml_categ_cpa) > 0) {
			...
			if ($data['cpa'] == 1) {
				if (isset($yandex_yml_categ_fee[$product['category_id']]) && $yandex_yml_categ_fee[$product['category_id']]!='') {
					$data['fee'] = $yandex_yml_categ_fee[$product['category_id']];
				}
			}

			$data['name'] = $product['manufacturer']." ".$this->getFieldValue($product, $this->config->get($this->CONFIG_PREFIX.'name_field'), 'name');
...
//				$data['manufacturer_warranty'] = 'true';
			...
			*/
			...
			if ($product['minimum'] > 1) {
				if (isset($data['sales_notes']))
					$data['sales_notes'].= ', минимальный заказ: '.$product['minimum'];
				else
					$data['sales_notes'] = 'Минимальный заказ: '.$product['minimum'];
			}
	...
	protected function setOptionedOffer($data, $product, $shop_currency, $offers_currency, $decimal_place) {
		...
		if (count($coptions)) {
			foreach ($coptions as $option) {
				...
				if ($this->numpictures>0 && $option['image']) {
	...
	*/
	...
	protected function setOffer($data) {
		...
		$allowed_tags = array_merge($allowed_tags, array('aliases'=>0, 'additional'=>0, 'description'=>0, 'sales_notes'=>0, 'promo'=>0, 'manufacturer_warranty'=>0, 'country_of_origin'=>0, 'downloadable'=>0, 'adult'=>0, /*'age'=>0,*/ 'barcode'=>0, 'keywords'=>0, 'rec'=>0));
